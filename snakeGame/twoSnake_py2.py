...
COLISIONSCREEN = False; COLISIONPARTNER = False; SELFCOLISION = True
...
class snake(cell):
	...
	def foodColision(self, food):
		...
		if isPointInsideRect(x, y, headRect):
			...
			return True
		return False
	def selfColision(self, colision = True):
		if colision == True:
...
def deathColision(screen, player, players):
	if player.selfColision(SELFCOLISION) or player.colisionScreen(COLISIONSCREEN):
