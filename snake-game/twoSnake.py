        ...
        if len(cells) > 2:
            for cell in cells[1:-1]:
                if cell.pointed[0] == True:
                    ...
                    bodySprite = sprites[3]  # snake point-curve
                    yInverse, bodyAngle = ANGLEMOTION[curve]

                    # shoulde be x, but is y. (WTF?)
                    if yInverse:
                        bodySprite = pygame.transform.flip(bodySprite,
                                                           False,
                                                           yInverse)
                else:
                ...
                bodyRect = cell.rect
                bodySpriteChanged = pygame.transform.rotate(bodySprite,
                                                            bodyAngle)
...
def score(screen, players, levelUp):
    ...
    scoreSurf = scoreText.render(upmessage, True, WHITE)
